...
class TestArtist:
    ...
    def test_construction(self):
        ...
        # when artist_id is not integer
        with pytest.raises(ValueError):
            Artist("7", "Drake")

        # when artist_id is less than 0
        with pytest.raises(ValueError):
            Artist(-5, "Drake")

        # when artist name is not string
        artist7 = Artist(7, 7)
        assert str(artist7) == '<Artist None, artist id = 7>'

        # when full name have trailing spaces
...
class TestUser:
    ...
    def test_construction(self):
        ...
        assert str(user1) == '<User amotys, password = amotys277>'
        assert str(user2) == '<User yunwi5, password = urrabbit978>'
